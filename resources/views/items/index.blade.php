...
function loadItems() {
    ...
    .then(res => {
        ...
        if (!res.data || res.data.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" class="text-center py-5 opacity-50">NO HAY ÍTEMS REGISTRADOS</td></tr>';
            return;
        }
        tbody.innerHTML = res.data.map(p => `
            <tr>
                ...
                <td class="font-bold" style="color:${p.stock_actual <= (p.stock_minimo || 0) ? '#dc3545' : '#007400'};">${parseFloat(p.stock_actual || 0).toFixed(2)}</td>
                <td class="font-bold">Bs. ${parseFloat(p.precio_compra || 0).toFixed(2)}</td>
                <td class="font-bold">Bs. ${parseFloat(p.precio_venta || 0).toFixed(2)}</td>
                <td>
